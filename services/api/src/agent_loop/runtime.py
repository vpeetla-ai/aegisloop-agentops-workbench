...
def evaluate(context: AgentContext) -> Evaluation:
    ...
    reasons = {
        ...
        "Cost and latency": "Runtime uses zero-dollar local/free-provider paths by default.",
...
def build_response(context: AgentContext, started: float) -> MissionResponse:
    evaluation = evaluate(context)
    context.artifacts["runtime_ms"] = round((perf_counter() - started) * 1000)
    response = MissionResponse(
        run_id=context.run_id,
        mission=context.request.mission,
        runtime=f"uv-fastapi/{context.request.mode}",
        cost_usd=0.0,
        artifact_markdown=context.artifacts.get("final_markdown", ""),
...
async def run_mission(request: MissionRequest) -> MissionResponse:
    ...
    response = build_response(context, started)
